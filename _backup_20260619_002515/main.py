...
async def fetch_top_coins(limit: int = 50) -> List[dict]:
    ...
    try:
        ...
        if not isinstance(data, list):
            # CoinGecko renvoie {"status":{...}} en cas de rate-limit -> garde l'ancien cache
            logger.warning(f"top_coins non-list response: {str(data)[:120]}")
...
async def fetch_close_series(symbol: str, days: int = 365) -> List[dict]:
    ...
    if not cg_id:
        ...
        for ck in ("top_coins_250", "top_coins_100", "top_coins_60", "top_coins_50", "memecoins"):
...
async def magic_band(symbol: str, period: str = "180d", user=Depends(get_current_user)):
    ...
    if last_i is not None:
        ...
        explanation = (
            ...
            f"Actuellement la tendance est {trend} et le prix est en « {zone.lower()} ». "
            f"Achetez plutôt quand le prix touche les bandes vertes du bas (survente) puis rebondit "
            f"— zone d'achat entre {p['l1']:.4g} et {p['l3']:.4g}, stop sous {p['l3']*0.97:.4g}. "
            f"Vendez/prenez vos bénéfices vers les bandes rouges du haut "
            f"(objectifs {p['basis']:.4g} → {p['u1']:.4g} → {p['u2']:.4g}). "
            f"Les flèches vertes/rouges sur le graphique sont les points d'achat/vente détectés."
        )
